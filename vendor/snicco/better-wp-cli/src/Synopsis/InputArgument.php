...
final class InputArgument implements InputDefinition
{
     ...
     */
    public function __construct(string $name, string $description = '', int $flags = self::REQUIRED, string $default = null, array $allowed_values = null)
    {
        ...
        if (null !== $this->allowed_values && !in_array($default, $this->allowed_values, \true)) {
            throw new InvalidArgumentException(sprintf('Default value [%s] is not in list of allowed values.', $default));
        }
        $this->default = $default;
    }
    /**
    ...
    {
        $args = ['type' => 'positional', 'name' => $this->name, 'description' => $this->description, 'optional' => $this->optional, 'repeating' => $this->repeating];
