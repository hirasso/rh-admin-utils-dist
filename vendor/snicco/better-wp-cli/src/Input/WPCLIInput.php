...
namespace RH\AdminUtils\Scoped\Snicco\Component\BetterWPCLI\Input;

use BadMethodCallException;
use InvalidArgumentException;
use RH\AdminUtils\Scoped\Snicco\Component\BetterWPCLI\Check;
...
final class WPCLIInput implements Input
{
     ...
     */
    public function __construct(Synopsis $synopsis, array $wp_cli_positional_args = [], array $wp_cli_assoc_args = [], $stream = null, bool $interactive = \true)
    {
        if (!Check::isListOfStrings($wp_cli_positional_args)) {
            throw new InvalidArgumentException('Received invalid arguments from wp-cli. Positional arguments should be a list of strings.');
        }
        /**
         * @var mixed $value
         */
        foreach ($wp_cli_assoc_args as $name => $value) {
            if (is_numeric($name)) {
                throw new InvalidArgumentException('Received invalid arguments from wp-cli. Assoc argument keys should not be numerical.');
            }
            if (is_bool($value)) {
                ...
                throw new InvalidArgumentException('Received invalid arguments from wp-cli. Assoc arguments should be all be of type array<string,string|bool>');
            }
        }
        ...
        if (count($wp_cli_positional_args) > count($positional) && !$synopsis->hasRepeatingPositionalArgument()) {
            throw new InvalidArgumentException(sprintf('Received [%s] positional arguments from wp-cli but synopsis only has [%s] positional arguments.', count($wp_cli_positional_args), count($positional)));
        }
    ...
    public function getArgument(string $name, ?string $default = null): ?string
    {
        if (isset($this->named_args_repeating[$name])) {
            throw new BadMethodCallException(sprintf('Positional argument [%s] is repeating. Use %s instead.', $name, self::class . '::getRepeatingArgument()'));
        }
    ...
    public function getRepeatingArgument(string $name, ?array $default = null): ?array
    {
        if (isset($this->named_args[$name])) {
            throw new BadMethodCallException(sprintf('Positional argument [%s] is not repeating. Use %s instead.', $name, self::class . '::getArgument()'));
        }
