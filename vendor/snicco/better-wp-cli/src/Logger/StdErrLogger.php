...
namespace RH\AdminUtils\Scoped\Snicco\Component\BetterWPCLI\Logger;
...
final class StdErrLogger implements Logger
{
    ...
    {
        $message = sprintf("%s/better-wp-cli.CRITICAL: Error thrown while running command [%s].\n\tMessage: %s\n\tException: %s at %s on line %s.", $this->log_prefix, $command_name, $e->getMessage(), get_class($e), $e->getFile(), $e->getLine());
        error_log($message);
    }
    public function logCommandFailure(int $exit_code, string $command_name, Input $input): void
    {
        $message = sprintf('%s/better-wp-cli.DEBUG: Command [%s] exited with code [%s].', $this->log_prefix, $command_name, $exit_code);
        error_log($message);
    }
}
