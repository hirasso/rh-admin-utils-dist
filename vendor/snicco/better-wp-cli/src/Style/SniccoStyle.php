...
namespace RH\AdminUtils\Scoped\Snicco\Component\BetterWPCLI\Style;

use RH\AdminUtils\Scoped\Snicco\Component\BetterWPCLI\Exception\CouldNotReadHiddenInput;
use RH\AdminUtils\Scoped\Snicco\Component\BetterWPCLI\Exception\InvalidAnswer;
use RH\AdminUtils\Scoped\Snicco\Component\BetterWPCLI\Exception\MissingInput;
use RH\AdminUtils\Scoped\Snicco\Component\BetterWPCLI\Input\Input;
use RH\AdminUtils\Scoped\Snicco\Component\BetterWPCLI\Output\BufferedOutput;
...
use RH\AdminUtils\Scoped\Snicco\Component\BetterWPCLI\Question\Question;
use function fgets;
...
final class SniccoStyle
{
    ...
    public function __construct(Input $input, Output $output, bool $write_to_stderr_if_available = \true, bool $is_windows = null, bool $tty_available = null)
    {
        $this->input = $input;
    ...
    {
        return $this->ask($question, '', \true);
    }
    public function confirm(string $question, bool $default = \true, int $attempts = null): bool
    {
        ...
        return 'yes' === $this->askQuestion(new Question($question, $default_answer, $validator, $attempts, $normalizer));
    }
    ...
    private function block(iterable $messages, string $prefix, string $bg_color, string $text_color): void
    {
        ...
        foreach ($messages as $message) {
            ...
            $line_prefix = (0 === $count) ? $static_prefix : str_repeat(' ', $static_prefix_length);
