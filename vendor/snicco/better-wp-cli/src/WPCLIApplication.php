...
namespace RH\AdminUtils\Scoped\Snicco\Component\BetterWPCLI;

use Closure;
use ErrorException;
use LogicException;
use RH\AdminUtils\Scoped\Snicco\Component\BetterWPCLI\CommandLoader\CommandLoader;
use RH\AdminUtils\Scoped\Snicco\Component\BetterWPCLI\Input\Input;
use RH\AdminUtils\Scoped\Snicco\Component\BetterWPCLI\Input\WPCLIInput;
use RH\AdminUtils\Scoped\Snicco\Component\BetterWPCLI\Logger\Logger;
use RH\AdminUtils\Scoped\Snicco\Component\BetterWPCLI\Logger\StdErrLogger;
use RH\AdminUtils\Scoped\Snicco\Component\BetterWPCLI\Output\ConsoleOutput;
use RH\AdminUtils\Scoped\Snicco\Component\BetterWPCLI\Output\ConsoleOutputInterface;
use RH\AdminUtils\Scoped\Snicco\Component\BetterWPCLI\Output\Output;
use RH\AdminUtils\Scoped\Snicco\Component\BetterWPCLI\Style\BG;
use RH\AdminUtils\Scoped\Snicco\Component\BetterWPCLI\Style\SniccoStyle;
use RH\AdminUtils\Scoped\Snicco\Component\BetterWPCLI\Style\Terminal;
use RH\AdminUtils\Scoped\Snicco\Component\BetterWPCLI\Style\Text;
use RH\AdminUtils\Scoped\Snicco\Component\BetterWPCLI\Synopsis\Synopsis;
use Throwable;
use WP_CLI;
...
{
    private bool $auto_exit = \true;
    private CommandLoader $command_loader;
    private string $name;
    private bool $catch_exceptions = \true;
    ...
    public function registerCommands(callable $add_command = null): void
    {
        ...
        foreach ($this->command_loader->commands() as $command_class) {
            ...
            $add_command($name, $callback, ['shortdesc' => $command_class::shortDescription(), 'longdesc' => $command_class::longDescription(), 'synopsis' => $synopsis->toArray(), 'when' => $command_class::when()]);
        }
        $this->commands_registered = \true;
    }
    /**
     * @internal
     *
    ...
    public function runCommand(Input $input, Output $output, Command $command): int
    {
        ...
        try {
            ...
            if ($exit_code > 255 || $exit_code < 0) {
                throw new LogicException(sprintf('Exit code must be between 0 and 255. Command [%s] returned [%s]', $command_name, $exit_code));
            }
    ...
    private function isInteractive(array $assoc_args): bool
    {
        ...
        return \true === $assoc_args['interaction'];
    }
    ...
    private function renderThrowable(Throwable $e, Input $input, Output $output): void
    {
        ...
        do {
            $output->newLine();
            $title = $style->colorize(sprintf('In %s line %d:', basename($e->getFile()), $e->getLine()), Text::YELLOW);
            ...
            if ($output->isVeryVerbose()) {
                ...
                foreach ($trace as $single_trace) {
                    ...
                    $output->writeln(sprintf(' %s%s at %s:%s', $style->colorize($class, Text::GREEN), $style->colorize(('' !== $function) ? $type . $function . '()' : '', Text::GREEN), $file, $line), Verbosity::QUIET);
                }
            }
    ...
    private function stringWidth(string $title): int
    {
        ...
        if (\false === $encoding = mb_detect_encoding($title, null, \true)) {
            return strlen($title);
        }
        // @codeCoverageIgnoreEnd
