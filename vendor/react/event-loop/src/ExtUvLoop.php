...
final class ExtUvLoop implements LoopInterface
{
    ...
    {
        if (!\function_exists('uv_loop_new') && !\function_exists('RH\AdminUtils\Scoped\uv_loop_new')) {
    ...
    {
        if (!isset($this->streamEvents[(int) $stream])) {
            $this->streamEvents[(int) $stream] = \RH\AdminUtils\Scoped\uv_poll_init_socket($this->uv, $stream);
        }
