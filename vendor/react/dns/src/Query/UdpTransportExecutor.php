...
namespace RH\AdminUtils\Scoped\React\Dns\Query;

use RH\AdminUtils\Scoped\React\Dns\Model\Message;
use RH\AdminUtils\Scoped\React\Dns\Protocol\BinaryDumper;
use RH\AdminUtils\Scoped\React\Dns\Protocol\Parser;
use RH\AdminUtils\Scoped\React\EventLoop\Loop;
use RH\AdminUtils\Scoped\React\EventLoop\LoopInterface;
use RH\AdminUtils\Scoped\React\Promise\Deferred;
/**
...
final class UdpTransportExecutor implements ExecutorInterface
{
    ...
    public function query(Query $query)
    {
        ...
        if (isset($queryData[$this->maxPacketSize])) {
            return \RH\AdminUtils\Scoped\React\Promise\reject(new \RuntimeException('DNS query for ' . $query->describe() . ' failed: Query too large for UDP transport', \defined('SOCKET_EMSGSIZE') ? \SOCKET_EMSGSIZE : 90));
        }
        ...
        if ($socket === \false) {
            return \RH\AdminUtils\Scoped\React\Promise\reject(new \RuntimeException('DNS query for ' . $query->describe() . ' failed: Unable to connect to DNS server ' . $this->nameserver . ' (' . $errstr . ')', $errno));
        }
        ...
        if ($written !== \strlen($queryData)) {
            return \RH\AdminUtils\Scoped\React\Promise\reject(new \RuntimeException('DNS query for ' . $query->describe() . ' failed: Unable to send query to DNS server ' . $this->nameserver . ' (' . $errstr . ')', $errno));
        }
        $loop = $this->loop;
