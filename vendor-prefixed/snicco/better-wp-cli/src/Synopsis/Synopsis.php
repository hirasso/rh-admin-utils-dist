...
namespace RH\AdminUtils\Snicco\Component\BetterWPCLI\Synopsis;
...
final class Synopsis
{
    ...
    {
        return array_values(array_map(fn(InputDefinition $definition): array => $definition->toArray(), $this->definitions));
    }
    /**
    ...
    private function add($definitions): void
    {
        ...
        foreach ($definitions as $definition) {
            if (!$definition instanceof InputDefinition) {
                throw new InvalidArgumentException(sprintf('%s is not an instance of %s', is_object($definition) ? get_class($definition) : gettype($definition), InputDefinition::class));
            }
            $name = $definition->name();
            if (isset($this->definitions[$name])) {
                throw new InvalidArgumentException(sprintf('Duplicate input name [%s] is not allowed in synopsis.', $name));
            }
            if ($definition instanceof InputArgument) {
                if (null !== $this->repeating_and_positional) {
                    throw new InvalidArgumentException(sprintf('Positional argument [%s] can not be added after repeating positional argument [%s].', $name, $this->repeating_and_positional->name()));
                }
                if ($this->optional_and_positional && !$definition->isOptional()) {
                    throw new InvalidArgumentException(sprintf('Required argument [%s] can not be added after optional argument [%s].', $name, $this->optional_and_positional->name()));
                }
