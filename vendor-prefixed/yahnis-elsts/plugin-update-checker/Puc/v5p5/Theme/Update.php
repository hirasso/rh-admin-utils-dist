...
use RH\AdminUtils\YahnisElsts\PluginUpdateChecker\v5p5\Update as BaseUpdate;
        ...
        protected function validateMetadata($apiResponse)
        {
            ...
            foreach ($required as $key) {
                if (!isset($apiResponse->{$key}) || empty($apiResponse->{$key})) {
                    return new \RH\AdminUtils\WP_Error('tuc-invalid-metadata', sprintf('The theme metadata is missing the required "%s" key.', $key));
                }
            }
            return \true;
        }
