...
class ACFOembedCache
{
    ...
    {
        if (class_exists('RH\AdminUtils\Scoped\RAH\ThemeBase\OembedHelper')) {
    ...
    public static function register_cache_post_type(): void
    {
        ...
        register_post_type($post_type, ['public' => \false, 'show_ui' => current_user_can('administrator'), 'labels' => ['name' => 'oEmbed Cache'], 'supports' => ['nothing'], 'menu_position' => 1000, 'show_in_menu' => "edit.php?post_type=acf-field-group", 'show_in_rest' => \false]);
    ...
    {
        $post_types = array_filter($post_types, fn($post_type) => $post_type !== self::$cache_post_type);
        return $post_types;
    }
    /**
    ...
    public static function meta_boxes(): void
    {
        ...
        \add_meta_box(id: 'rhau-oembed-cache-metabox', title: __('RH Admin Utils: Global oEmbed cache', RHAU_TEXT_DOMAIN), callback: [__CLASS__, 'render_custom_meta_box'], screen: 'rhau-oembed-cache"', context: 'normal', priority: 'high');
    }
    ...
    public static function format_value_oembed($value, $post_id, $field)
    {
        ...
        if (str_contains($value, '<iframe') && preg_match('/(vimeo.com|youtube.com)/', $value)) {
    ...
    private static function acf_oembed_get(mixed $value, string|int $post_id, array $field): string|false|null
    {
        ...
        global $wp_embed;
        $attr = ['width' => $field['width'], 'height' => $field['height']];
        remove_filter('embed_oembed_html', 'RH\AdminUtils\Scoped\Roots\Soil\CleanUp\embed_wrap');
        /**
        ...
        add_filter('embed_oembed_html', 'RH\AdminUtils\Scoped\Roots\Soil\CleanUp\embed_wrap');
        return $html ?: $value;
    }
    /**
    ...
    {
        $query = new \WP_Query(['post_type' => self::$cache_post_type, 'posts_per_page' => 1, 'suppress_filters' => \true, 'fields' => 'ids']);
        $post_id = $query->posts[0] ?? wp_insert_post(['post_title' => 'oEmbed cache', 'post_type' => self::$cache_post_type, 'post_status' => 'publish']);
    ...
    {
        $cache_entries = array_filter(get_post_meta($post->ID) ?: [], fn($key) => str_starts_with($key, '_oembed_') && !str_starts_with($key, '_oembed_time_'), \ARRAY_FILTER_USE_KEY);
        $cache_entries = array_map(fn($item) => esc_html($item[0]), $cache_entries);
        ?>
        <style>
        ...
        <p>
            <?php 
            _e('There global ACF oEmbed cache is currently empty.', RHAU_TEXT_DOMAIN);
            ?>
        </p>
        <?php 
        } else {
        ...
        <p>
            <?php 
            printf(__('There are %d oEmbed responses cached globally.', RHAU_TEXT_DOMAIN), count($cache_entries));
            ?>
        </p>
        <pre id="rhau-oembed-cache-output"><?php 
            echo var_dump($cache_entries);
            ?></pre>

        <a
            class="button button-primary button-large"
            href="<?php 
            echo get_delete_post_link(get_post(), null, \true);
            ?>">
            Flush oEmbed Cache
        </a>
