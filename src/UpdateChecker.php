...
class UpdateChecker
{
    public static function init(string $entryPoint)
    {
        ...
        $checker = PucFactory::buildUpdateChecker("https://github.com/{$owner}/{$name}/", $entryPoint, $name);
    ...
    {
        if (defined('RHAU_GITHUB_TOKEN') && is_string(RHAU_GITHUB_TOKEN) && !empty(trim(RHAU_GITHUB_TOKEN))) {
